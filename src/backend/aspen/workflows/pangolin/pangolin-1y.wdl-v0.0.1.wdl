...
# Variant of pangolin.wdl-v0.0.1.wdl that restricts find_samples.py to samples
# with collection_date within the last 365 days. All other behavior is identical
# to the canonical pangolin WDL. Intended for manual SFN invocation only; the
# scheduled EventBridge target continues to point at the unfiltered WDL.
...
workflow pangolin {
    input {
        ...
        Int max_collection_age_days = 365
    }
...
task pangolin_workflow {
    ...
    /usr/local/bin/python3 find_samples.py \
        ...
        --max-collection-age-days ~{max_collection_age_days}
    ./run_pangolin.sh
    >>>
